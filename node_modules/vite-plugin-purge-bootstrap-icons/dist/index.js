...
export default (config = {}) => {
    return {
        ...
        async writeBundle(options, bundle) {
            const files = Object.values(bundle);
            const fonts = files
                .filter((f) => f.name === 'bootstrap-icons.woff' || f.name === 'bootstrap-icons.woff2');
            const css = files
            ...
            await Promise.all(fonts.map(async (file) => {
                ...
                const newGlyphs = new Array();
                classList.map((className, i) => {
                    const glyphsAsArray = Object.values(font.glyphs['glyphs']);
                    const foundGlyph = glyphsAsArray.find((glyph) => glyph.name === className);
                    if (!foundGlyph) {
                        throw new Error(`Glyph ${className} not found`);
                    }
